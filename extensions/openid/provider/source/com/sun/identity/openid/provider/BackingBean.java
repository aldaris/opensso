...
public class BackingBean
{
    ...
    private enum Point { BEFORE, AFTER };
    ...
    private void dispatch(PhaseEvent event, Point point)
    {
        ...
        try
        {
            if (id.equals(PhaseId.APPLY_REQUEST_VALUES)) {
                switch (point) {
                    case BEFORE: beforeApplyRequestValues(event); break;
                    case AFTER: afterApplyRequestValues(event); break;
                }
            }

            else if (id.equals(PhaseId.PROCESS_VALIDATIONS)) {
                switch (point) {
                    case BEFORE: beforeProcessValidations(event); break;
                    case AFTER: afterProcessValidations(event); break;
                }
            }

            else if (id.equals(PhaseId.UPDATE_MODEL_VALUES)) {
                switch (point) {
                    case BEFORE: beforeUpdateModelValues(event); break;
                    case AFTER: afterUpdateModelValues(event); break;
                }
            }

            else if (id.equals(PhaseId.INVOKE_APPLICATION)) {
                switch (point) {
                    case BEFORE: beforeInvokeApplication(event); break;
                    case AFTER: afterInvokeApplication(event); break;
                }
            }

            else if (id.equals(PhaseId.RENDER_RESPONSE)) {
                switch (point) {
                    case BEFORE: beforeRenderResponse(event); break;
                    case AFTER: afterRenderResponse(event); break;
                }
            }
        }
    ...
    {
// TODO: provide redirect error response (openid.mode=error,openid.error=msg)
        try {
     ...
     * @param event TODO.
     * @throws BadRequestException TODO.
     * @throws InternalServerException TODO.
     */
