 ...
 *
 * $Id: OCSPResponse.java,v 1.1 2007-10-22 15:06:31 beomsuk Exp $
 *
    ...
    OCSPResponse(byte[] bytes, PKIXParameters params,
        ...
        try {
            ...
            if (debug.messageEnabled()) {
                debug.message("OCSPResponse.OCSPResponse: responseStatus" +
            ...
            if (responseType.equals((Object)OCSP_BASIC_RESPONSE_OID)) {
                ...
                if (debug.messageEnabled()) {
                    debug.message("OCSPResponse.OCSPResponse: type: " +
                                  responseType);
                }
            ...
            if (tag == NAME_TAG) {
                ...
                if (debug.messageEnabled()) {
                    debug.message("OCSPResponse.OCSPResponse: Responder name: "
                                  + responderName);
                }
            } else if (tag == KEY_TAG) {
                // Ignore, for now
            ...
            if (seqDerIn.available() > 0) {
                ...
                if (seq.isContextSpecific((byte)1)) {
                    ...
                    for (int i = 0; i < responseExtDer.length; i++) {
                        ...
                        if (debug.messageEnabled()) {
                            debug.message("OCSPResponse.OCSPResponse: " +
                                          "extension: " +
                                          responseExtension[i]);
                        }
                        if ((responseExtension[i].getExtensionId()).equals(
                            (Object)OCSP_NONCE_EXTENSION_OID)) {
                            ocspNonce =
            ...
            if (x509Certs[0] != null) {
                ...
                // which was set locally.
                    ...
                    if (keyPurposes == null ||
                        ...
                        if (debug.messageEnabled()) {
                            debug.message("OCSPResponse.OCSPResponse: " +
                                          "Responder's certificate is not " +
            ...
            if (responderCert != null) {
                ...
                if (! verifyResponse(responseDataDer, responderCert,
                    ...
                    if (debug.messageEnabled()) {
                        debug.message("OCSPResponse.OCSPResponse: Error " +
                                      "verifying OCSP Responder's signature");
                    }
                    throw new CertPathValidatorException(
                ...
                if (debug.messageEnabled()) {
                    debug.message("OCSPResponse.OCSPResponse: Unable to " +
                                  "verify OCSP Responder's signature");
                }
                throw new CertPathValidatorException(
    ...
    private boolean verifyResponse(byte[] responseData, X509Certificate cert,
        ...
        try {
            ...
            if (respSignature.verify(signBytes)) {
                if (debug.messageEnabled()) {
                    debug.message("OCSPResponse.verifyResponse: Verified " +
                                  "signature of OCSP Responder");
                }
                return true;

            } else {
                if (debug.messageEnabled()) {
                    debug.message("OCSPResponse.verifyResponse: Error " +
                                  "verifying signature of OCSP Responder");
                }
                return false;
            }
        ...
        private SingleResponse(DerValue der) throws IOException {
            ...
            if (tag ==  CERT_STATUS_GOOD) {
                ...
                if (debug.messageEnabled()) {
                    ...
                    debug.message("OCSPResponse.SingleResponse.SingleResponse:"+
