...
Service::Service(const char *svcName,
    ...
    if(fetchSessionAttrs) {
        sessionAttributesMap = new Properties(false);
    ...
    if(fetchResponseAttrs) {
        responseAttributesMap = new Properties(false);
				...
				const std::vector<PDRefCntPtr>& results) const
{
     ...
     * resource.
     */
    if(fetchProfileAttrs || fetchSessionAttrs || fetchResponseAttrs || mUserIdParamType == USER_ID_PARAM_TYPE_LDAP) {
		...
		if((mFetchFromRootResource == true) && !resObj.getResourceRoot(rsrcTraits, rootRes)) {
			Log::log(logID, Log::LOG_WARNING,
