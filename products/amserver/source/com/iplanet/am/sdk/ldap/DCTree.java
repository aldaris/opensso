 ...
 *
 * $Id: DCTree.java,v 1.1 2005-11-01 00:29:29 arvindp Exp $
 *
...
public class DCTree implements AMConstants, DCTreeInterface {
    ...
    // private Hashtable domainToOrgTable;
    static {
        dcTreeStartDN = SystemProperties.get("com.iplanet.am.domaincomponent");
        ...
        try {
            if (Compliance.gsc == null) {
                ServiceSchemaManager scm = new ServiceSchemaManager(
                        ADMINISTRATION_SERVICE, CommonUtils.getInternalToken());
                Compliance.gsc = scm.getGlobalSchema();
            }
            Map attrMap = Compliance.gsc.getAttributeDefaults();
        ...
        if ((domainName != null) && (domainName != "")) {
            try {
                ...
                if (attrSetArray[1] != null) {
                    setDomainAttributes(
                            token, orgGuid.getDn(), attrSetArray[1]);
                }
    ...
    protected AttrSet[] splitAttrSet(String orgDN, AttrSet attrSet)
        ...
        if (debug.messageEnabled()) {
            debug.message("Compliance.splitAttrSet: " + "domain attrset = "
                    + attrArray[1].toString());
            debug.message("Compliance.splitAttrSet: " + "non-domain attrset = "
                    + attrArray[0].toString());
        }
        return attrArray;
    }
