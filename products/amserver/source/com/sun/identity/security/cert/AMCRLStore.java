...
public class AMCRLStore extends AMCertStore {
    ...
    private X509CRL getCRLFromEntry(LDAPEntry crlEntry) 
        ...
        if (crlEntry == null) {
         ...
         try {
            ...
            debug.error("AMCRLStore.getCRLFromEntry : " + 
    ...
    private CRLDistributionPointsExtension getCRLDPExt(
        ...
        if (extFactory != null) {
            try {
                ...
                debug.error("AMCRLStore.getCRLDPExt : " +
    ...
    private IssuingDistributionPointExtension getCRLIDPExt(X509CRL crl){
        ...
        if (extFactory != null) {
            try {
                ...
                debug.error("AMCRLStore.getCRLIDPExt : " +
                    "Error finding CRL distribution Point configured.", e);
            }   
        }
    ...
    private synchronized X509CRL 
        ...
        if (Crls != null) {
            try {
                ...
                debug.error("AMCRLStore.getUpdateCRLFromCrlDP : " +
                    "Error in generating X509CRL", e);
            }
        }
    ...
    private synchronized X509CRL 
        ...
        if (Crls != null) {
            try {
                ...
                debug.error("AMCRLStore.getUpdateCRLFromCrlIDP : " +
                    "Error in generating X509CRL", e);
            }
        }
    ...
    private byte[] getCRLsFromGeneralNames(GeneralNames gName) {
        ...
        do {
            ...
            if (debug.messageEnabled()) {
                debug.message("AMCRLStore.getCRLsFromGeneralNames : " +
                    "DP Name : " + uri);
            }
    ...
    private boolean updateCRL(LDAPConnection ldc, String dn, byte[] crls) {
        ...
        try {
            ...
            debug.error("AMCRLStore.updateCRL : " +
                "Error updating CRL Cache : ", e);
            return false;
        }
    ...
    private byte[] getCRLByLdapURI(String uri){
        ...
        try {
            ...
            if (results == null || results.hasMoreElements() == false) {
                debug.error("AMCRLStore.getCRLByLdapURI : " +
                    "No CRL found.");
                return null;
            }
    ...
    private byte[] getCRLByHttpURI(String url){
        ...
        try {
            ...
            if (uriParamsCRL != null) {
                ...
                while (st1.hasMoreTokens()) {
                    ...
                    if (st2.countTokens() == 2) {
                        ...
                        params.append(URLEncDec.encode(param) + "=" + 
            ...
            if (buffer.size() > 0) {
                crl = buffer.toByteArray();
            }
        } catch (Exception e) {
            debug.error("AMCRLStore.getCRLByLdapURI : " +
                "Error in getting CRL", e);
        }
