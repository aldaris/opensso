 ...
 *
 * $Id: AMEmailNotification.java,v 1.1 2005-11-01 00:29:27 arvindp Exp $
 *
...
public class AMEmailNotification {
    ...
    public AMEmailNotification(String userDN) {
        entryDN = userDN;
        ...
        while (iter.hasNext()) {
            ...
            if (emailStr.equals(self)) {
                ...
                if (emails == null || emails.isEmpty()) {
                    ...
                    debug.message("AMEmailNotification.sendMail(): " +
    ...
    private Set getNotificationList(String attributeName) {
        ...
        try {
            String organizationDN = dsManager.getOrganizationDN(internalToken,
                    this.organizationDN);
            ...
            debug.error("AMEmailNotification.getNotificationList() "
    ...
    public void sendUserCreateNotification(Map attributes) {
        try {
            ...
            if (debug.warningEnabled()) {
                debug.warning(
                        "AMEmailNotification.sendUserCreateNotification() " +
    ...
    public void sendUserDeleteNotification(Map attributes) {
        try {
            ...
            if (debug.warningEnabled()) {
                debug.warning(
                        "AMEmailNotification.sendUserDeleteNotification() " +
                        "Unable to send email for user: " + entryDN, e);
            }
        }
    }
    ...
    public void sendUserModifyNotification(SSOToken token, Map attributes,
        ...
        try {
                    ...
                    if (valuesChanged) {
                        while (stz.hasMoreTokens()) {
                            ...
                            if (email.equals(self)) {
                                ...
                                Map emailAttrMap = dsManager.getAttributes(
                                        token, entryDN, attrNamesSet,
                                        AMObject.USER);
            ...
            if (debug.warningEnabled()) {
                debug.warning(
                        "AMEmailNotification.sendUserModifyNotification() " +
                        "Unable to send email for user: " + entryDN, me);
            }
        } catch (SSOException e) {
            debug.error("AMEmailNotification.sendUserModifyNotification() "
