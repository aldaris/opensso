 ...
 *
 * $Id: Compliance.java,v 1.1 2005-11-01 00:29:28 arvindp Exp $
 *
 ...
 */
public class Compliance implements AMConstants, ComplianceInterface {
    ...
    static DirectoryManager dsManager;

    static SSOToken internalToken;
    static {
        // init();
        internalToken = CommonUtils.getInternalToken();
        dsManager = DirectoryManager.getInstance();
    }
    ...
    private String getGroupFromRoleDN(DN dn) {
        ...
        if (groupName == null) { // If not, a org level admin-role
            String roleName = 
                ((RDN) dn.getRDNs().firstElement()).getValues()[0];
    ...
    protected void verifyAndLinkRoleToGroup(SSOToken token, Guid[] membersGuid,
        ...
        if (groupName != null) { // roleDN corresponds to an admin role
            ...
            String groupDN = AMNamingAttrManager.getNamingAttr(AMObject.GROUP)
                    + "=" + groupName + ",ou=Groups," + orgDN;
            String groupRDN = AMNamingAttrManager.getNamingAttr(AMObject.GROUP)
                    + "=" + groupName;
            try {
                // Add the members to corresponding group.
                AssignableDynamicGroup group = 
                    (AssignableDynamicGroup) UMSObject.getObject(
                            token, new Guid(groupDN));
                ...
                for (int i = 0; i < numMembers; i++) {
                    addAttributesToEntry(
                            token, membersGuid[i].getDn(), attrSet);
                }
     ...
     * <Code>membersGuid</Code>
     * are added to the admin role.
     * 
     * @param token
     *            SSO Token
     * @param membersGuid
    ...
    protected void verifyAndLinkGroupToRole(SSOToken token, Guid[] membersGuid,
        ...
        if (roleName != null) { // roleDN corresponds to an admin role
            ...
            try {
                ...
                debug.error("Compliance.verifyAndLinkGroupToRole: "
                        + "Admin groups are missing");
            } catch (UMSException ue) {
    ...
    protected void verifyAndUnLinkGroupToRole(SSOToken token, Set members,
        ...
        if (roleName != null) {
            ...
            String roleDN = AMNamingAttrManager.getNamingAttr(AMObject.ROLE)
                    + "=" + roleName + "," + orgDN;
            if (debug.messageEnabled()) {
                debug.error("Compliance."
                        + "verifyAndUnlinkGroupToRole(): Unlinking group: "
                        + groupDN + " to role :" + roleDN);
            }
    ...
    public boolean isAncestorOrgDeleted(SSOToken token, String dn,
        ...
        if (profileType != AMObject.ORGANIZATION) {
            tdn = dsManager.getOrganizationDN(internalToken, dn);
        }
        while (!tdn.equalsIgnoreCase(rootSuffix)) {
            ...
            tdn = dsManager.getOrganizationDN(token, dn);
        }
    ...
    public void verifyAndDeleteObject(SSOToken token, String profileDN)
        ...
        try {
            ...
            if (po instanceof com.iplanet.ums.User) {
                ...
                po.save();
                mailer = new AMEmailNotification(profileDN);
                if (mailer != null) {
                    mailer.setUserDeleteNotificationList();
                    attributes = dsManager.getAttributes(token, profileDN,
                            AMObject.USER);
            ...
            if (po instanceof com.iplanet.ums.Organization) {
                ...
                po.modify(attr, ModSet.REPLACE);
                po.save();
                if (dsManager.dcTree.isRequired()) {
                    dsManager.dcTree.updateDomainStatus(token, profileDN,
                            "deleted");
                }
            } else {
     ...
     */
    public boolean isAdminGroupsEnabled(String orgDN) throws AMException {
    ...
    protected void createAdminGroups(SSOToken token, PersistentObject org)
        ...
        dsManager.createEntry(token, DOMAIN_ADMINISTRATORS,
        ...
        dsManager.createEntry(token, DOMAIN_HELP_DESK_ADMINISTRATORS,
                AMObject.ASSIGNABLE_DYNAMIC_GROUP, gcDN, attributes);
    }
     ...
     *                if an error is encountered
     */
    public boolean isComplianceUserDeletionEnabled() throws AMException {
        try {
            if (gsc == null) {
                ServiceSchemaManager scm = new ServiceSchemaManager(
                        ADMINISTRATION_SERVICE, internalToken);
