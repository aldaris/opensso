<%--
   ...
   $Id: fedletDefault.jsp,v 1.1 2008-03-26 04:31:41 qcheng Exp $
...
<%
    String respXML = new String(Base64.decode((String) request.getAttribute(
    ...
    if (respXML != null && respXML.length() != 0) {
        ...
        List lst = assertion.getAuthnStatements();
        if (lst.size() > 0) {
        ...
        lst = assertion.getAttributeStatements();
        if (lst.size() > 0) {
            AttributeStatement attrStat = (AttributeStatement) 
            ...
            out.println("<br><b>Attributes: </b>");
            List attrList = attrStat.getAttribute();
            Iterator iter = attrList.iterator();
            while (iter.hasNext()) {
                Attribute attr = (Attribute) iter.next();
                out.println("<br>name=" + attr.getName() + " value=" 
                    + attr.getAttributeValueString().get(0));
            }
        }
    } else {
	%>
	    <%= SAML2SDKUtils.bundle.getString("missingMessageParam") %>
	<%
    }
%>
<script>
